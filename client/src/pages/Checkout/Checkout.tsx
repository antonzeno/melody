...
const Checkout = () => {
    ...
    useEffect(() => {
        state.clientSecret === '' &&
            axios.post("https://api.stripe.com/v1/payment_intents", new URLSearchParams({ 'amount': '2000', 'currency': 'usd' }).toString(), {
                headers: {
                    "Content-Type": "application/x-www-form-urlencoded",
                    "Authorization": `Bearer ${process.env.REACT_APP_STRIPE_SECRET}`
                },
            }).then((response) => {
                setState(prevState => ({ ...prevState, clientSecret: response.data.client_secret }));
            }).catch((error) => {
        ...
        <div className='container bg-dark shadow shadow-md p-3'>
            <div className="row">
                ...
                <div className="col-12 col-md-6">
                    {!state.loading && <Elements stripe={stripePromise} options={options}>
